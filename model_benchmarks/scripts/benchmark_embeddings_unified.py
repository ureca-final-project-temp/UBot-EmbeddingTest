...
측정 지표:
  ...
  성능   - 평균 임베딩 시간 / P95 임베딩 시간 / 초당 처리량(단일 요청 기준)
...
사용법:
  python benchmark_embeddings_unified.py --xlsx EmbeddingTestFAQ_확장.xlsx
...
def split_ids(cell_value) -> list[str]:
    """'FAQ-051; FAQ-010' 같은 셀 값을 ['FAQ-051', 'FAQ-010']으로 분리한다."""
    if not cell_value:
        return []
...
def load_test_queries(xlsx_path: str) -> list[dict]:
    """6개 Retrieval 시트를 모두 읽어서 하나의 쿼리 리스트로 합친다."""
    wb = openpyxl.load_workbook(xlsx_path, data_only=True)
    ...
    for sheet_name, category_label in QUERY_SHEETS.items():
        ...
        ws = wb[sheet_name]
        for row in ws.iter_rows(min_row=5, values_only=True):
            test_id, user_query, gt, acceptable, hard_neg = row[0], row[1], row[2], row[3], row[4]
            if not test_id:
                continue
            queries.append(
                {
                    ...
                    "query": user_query,
                    "gt_ids": split_ids(gt),
                    "acceptable_ids": split_ids(acceptable) or split_ids(gt),
                    "hard_neg_ids": split_ids(hard_neg),
                }
            )
    return queries
...
def evaluate_model(config: dict, faq_corpus: dict[str, str], queries: list[dict]) -> dict:
    ...
    faq_vectors = {}
    for i, (faq_id, question) in enumerate(faq_corpus.items(), start=1):
        vec, _ = embedder.embed(question, mode="passage")
        faq_vectors[faq_id] = vec
        if i % 20 == 0:
            print(f"  {i}/{len(faq_corpus)} 완료")
    ...
    # 4) 집계 (전체 + 카테고리별)
    summary = summarize(name, per_query_results, latencies, memory_info)
...
def summarize(model_name: str, results: list[dict], latencies: list[float], memory_info: dict) -> list[dict]:
